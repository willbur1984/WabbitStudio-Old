...
static int find_header(u_char (*dest)[PAGE_SIZE], int page, int ident1, int ident2) {
	int i;
...
static symbol83P_t *search_symlist(symlist_t *symlist, const TCHAR *name, size_t name_len) {
...
TCHAR *Symbol_Name_to_String(int model, symbol83P_t *sym, TCHAR *buffer) {
	...
	if (model == TI_86) {
		...
		switch(sym->type_ID) {
			...
			case PictObj:
				strcpy(buffer, _T("Pic%d"), circ10(sym->name[1]));
				return buffer;
			case GDBObj:
				strcpy(buffer, _T("GDB%d"), circ10(sym->name[1]));
				return buffer;
			case StrngObj:
				strcpy(buffer, _T("Str%d"), circ10(sym->name[1]));
				return buffer;		
			case RealObj:
			case CplxObj:
				strcpy(buffer, _T("%c"), sym->name[0]);
				return buffer;
			case ListObj:
			case CListObj:
				if ((u_char) sym->name[1] < 6) {
					strcpy(buffer, _T("L%d"), sym->name[1] + 1); //L1...L6
				} else {
					strcpy(buffer, _T("%s"), sym->name + 1); // No Little L
				}
				return buffer;
			case MatObj:
				if (sym->name[0] == 0x5C) {
					strcpy(buffer, _T("[%c]"), 'A' + sym->name[1]);
					return buffer;
				}
				return NULL;
		//	case EquObj:
			...
			case EquObj_2:
				{
					...
					switch(sym->name[1] & 0xF0) {
						case 0x10: //Y1
							strcpy(buffer, _T("Y%d"),circ10(b));
							return buffer;
						case 0x20: //X1t Y1t
							strcpy(buffer, _T("X%dT"), ((b/2)+1)%6);
							if (b % 2) {
								buffer[0] = 'Y';
							}
							return buffer;
						case 0x40: //r1
							strcpy(buffer, _T("R%d"),(b+1)%6);
							return buffer;
						case 0x80: //Y1
							switch (b) {
								case 0: 
...
static void SetRealAns(CPU_t *cpu, TCHAR *text) {
