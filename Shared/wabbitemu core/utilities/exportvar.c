...
static int mprintf(MFILE* mf, const char *format, ...) {
	unsigned char *temp;
	if (!mf) return EOF;
	va_list list;
...
static void AddrOffset(int *page, unsigned int *address, int offset) {
	if (*page) {
		int pi = (offset >= 0) ? 1 : -1;
		*address += offset;
...
static int VarRead(LPCALC lpCalc, int page, unsigned int address) {
	unsigned char mem;
